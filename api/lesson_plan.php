...
$result = $conn->query("SELECT e.emp_id, e.name as emp_name, d.name as dept_name, 
                         p.day, p.period, p.topic, p.description, p.unit, p.plan_date,
...
if ($format === 'csv') {
    ...
    echo "Employee ID,Employee Name,Department,Day,Period,Class,Subject Code,Subject Name,Topic,Unit,Date\n";
    foreach ($plans as $p) {
        echo "{$p['emp_id']},{$p['employee_name']},{$p['department']},{$p['day']},{$p['period']},{$p['class']},{$p['subject_code']},{$p['subject_name']},{$p['topic']},{$p['unit']},{$p['date']}\n";
    }
} else {
